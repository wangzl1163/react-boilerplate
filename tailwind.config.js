const colors = require("tailwindcss/colors");
const defaultTheme = require("tailwindcss/defaultTheme");
const theme = require("./src/Assets/Theme/Theme.json");

/** @type {import('tailwindcss').Config} */
module.exports = {
   important: false,
   content: ["./index.html", "./src/main.jsx", "./src/Components/**/*.jsx", "./src/Layout/**/*.jsx", "./src/Pages/**/*.jsx"],
   corePlugins: [
      // 启用的核心插件/工具类
      "margin",
      "padding",
      "backgroundColor",
      "alignItems",
      "alignSelf",
      "backdropFilter",
      "backdropBlur",
      "backgroundImage",
      "backgroundOpacity",
      "backgroundOrigin",
      "backgroundPosition",
      "backgroundRepeat",
      "backgroundSize",
      "borderColor",
      "borderOpacity",
      "borderRadius",
      "borderStyle",
      "borderWidth",
      "boxShadow",
      "boxShadowColor",
      "boxSizing",
      "cursor",
      "display",
      "fill",
      "flex",
      "flexBasis",
      "flexDirection",
      "flexShrink",
      "flexGrow",
      "flexWrap",
      "float",
      "fontFamily",
      "fontSize",
      "fontStyle",
      "fontWeight",
      "gap",
      "gradientColorStops",
      "gradientColorStopPositions",
      "height",
      "inset",
      "justifyContent",
      "justifyItems",
      "justifySelf",
      "lineHeight",
      "listStyleType",
      "maxHeight",
      "maxWidth",
      "minHeight",
      "minWidth",
      "opacity",
      "order",
      "overflow",
      "placeholderColor",
      "position",
      "scale",
      "strokeWidth",
      "stroke",
      "space",
      "textAlign",
      "textColor",
      "textDecoration",
      "textDecorationStyle",
      "textIndent",
      "textOverflow",
      "textTransform",
      "transform",
      "transformOrigin",
      "transitionDelay",
      "transitionDuration",
      "transitionProperty",
      "transitionTimingFunction",
      "translate",
      "verticalAlign",
      "visibility",
      "whitespace",
      "width",
      "zIndex",
      "wordBreak"
   ],
   theme: {
      // 覆盖/屏蔽默认主题配置
      colors: {
         inherit: "inherit",
         transparent: "transparent",
         current: "currentColor",
         black: colors.black,
         white: {
            100: colors.white,
            85: "rgba(255, 255, 255, 0.85)",
            60: "rgba(255, 255, 255, 0.6)"
         },
         brand: {
            base: theme.token.colorPrimary, // 主色
            "light-1": "var(--color-primary-light-1)",
            "light-2": "var(--color-primary-light-2)",
            "light-3": "var(--color-primary-light-3)",
            "light-4": "var(--color-primary-light-4)",
            "light-5": "var(--color-primary-light-5)",
            "dark-1": "var(--color-primary-dark-1)",
            "dark-2": "var(--color-primary-dark-2)",
            "dark-3": "var(--color-primary-dark-3)",
            "dark-4": "var(--color-primary-dark-4)"
         },
         font: {
            header: theme.components.Typography.colorTextHeading,
            primary: theme.components.Typography.colorText,
            // regular: "var(--text-color-regular)",
            secondary: theme.token.colorTextSecondary,
            placeholder: theme.token.colorTextTertiary,
            disabled: theme.components.Button.colorTextDisabled,
            action: "#6A81FF",
            auxiliary: "#A6ABBF" // 辅助文字颜色
         },
         line: {
            darker: "var(--border-color-darker)",
            dark: "var(--border-color-dark)",
            base: "var(--border-color)",
            light: "var(--border-color-light)",
            lighter: "var(--border-color-lighter)",
            "extra-light": "var(--border-color-extra-light)"
         },
         fill: {
            darker: "var(--fill-color-darker)",
            dark: "var(--fill-color-dark)",
            base: "var(--fill-color)",
            light: "var(--fill-color-light)",
            lighter: "var(--fill-color-lighter)",
            "extra-light": "var(--fill-color-extra-light)"
         },
         "page-bg": "#0C121E",
         mask: "var(--mask-color)",
         module: {
            base: "#252E3F",
            dark: "#1C2233"
         }
      },
      screens: {},
      spacing: {
         ...defaultTheme.scale,
         0: "0px",
         px: "1px",
         0.5: "2px",
         1: "4px",
         1.5: "6px",
         2: "8px",
         2.5: "10px",
         3: "12px",
         4: "16px",
         4.5: "18px",
         5: "20px",
         5.5: "22px",
         6: "24px",
         7: "28px",
         7.5: "30px",
         8: "32px",
         8.5: "34px",
         9: "36px",
         10: "40px",
         11: "44px",
         11.5: "46px",
         12: "48px",
         13: "52px",
         13.5: "54px",
         14: "56px",
         15: "60px",
         15.5: "62px",
         16: "64px",
         17: "68px",
         18: "72px",
         19: "76px",
         19.5: "78px",
         20: "80px",
         23: "92px",
         24: "96px",
         25: "100px",
         28.5: "114px",
         29: "116px",
         30: "120px",
         32: "128px",
         32.5: "130px",
         33: "132px",
         34: "136px",
         35: "140px",
         36: "144px",
         36.5: "146px",
         37: "148px",
         37.5: "150px",
         38: "152px",
         38.5: "154px",
         39: "156px",
         39.5: "158px",
         40: "160px",
         41: "164px",
         42: "168px",
         42.5: "170px",
         42.75: "171px",
         43.75: "175px",
         44: "176px",
         46: "184px",
         47: "188px",
         48: "192px",
         48.5: "194px",
         50: "200px",
         52: "208px",
         56: "224px",
         60: "240px",
         61: "244px",
         62: "248px",
         65: "260px",
         70: "280px",
         72.25: "289px",
         81: "324px",
         83.5: "334px",
         87: "348px",
         87.5: "350px",
         88.5: "354px",
         92.5: "370px",
         93: "372px",
         95: "380px",
         100: "400px",
         116: "464px",
         120: "480px",
         134.5: "538px",
         147: "588px",
         150: "600px"
      },
      fontSize: {
         "extra-sm": ["12px", "16px"],
         sm: ["13px", "18px"],
         base: ["14px", "20px"],
         me: ["16px", "24px"],
         lg: ["20px", "32px"],
         "extra-lg": ["24px", "32px"],
         xl: ["28px", "34px"],
         "extra-xl": ["32px", "40px"],
         xxl: ["36px", "44px"],
         xxxl: ["40px", "56px"],
         DEFAULT: "14px"
      },
      fontFamily: { inherit: "inherit", mono: defaultTheme.fontFamily.mono },
      extend: {
         // 继承默认主题的配置
         lineHeight: ({ theme }) => ({
            initial: "initial",
            ...theme("spacing")
         }),
         flexBasis: {
            fit: "fit-content"
         },
         borderRadius: ({ theme }) => ({
            ...theme("spacing")
         })
      }
   },
   plugins: []
};
